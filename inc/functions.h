...
char							*f_strjoin(t_game *game, char const *s1,
									char const *s2);
size_t							f_strlen(const char *s);
int								f_char_count(char *str, char c);
char							*f_strchr(const char *s, int c);
void							*f_memcpy(void *dest, const void *src,
									size_t n);
...
void							f_check_args(t_game *game, int argc,
									char **argv);
...
void							f_set_texture_path(t_game *game,
									t_line_type type, char *line);
...
t_coords						f_next_wall_hit(t_game *game, t_coords current,
									float angle);
float							f_cosine_distance(t_coords a, t_coords b,
									float angle1, float angle2);
int								f_get_tex_color(t_game *game, t_coords wall_hit,
									float wall_height_ratio);
int								f_handle_mouse(int x, int y, void *param);
int								f_get_pixel(t_game *game, t_tex *tex,
									t_coords pos);
char							f_determine_tile_type(t_game *game,
									t_coords wall_hit, t_dir dir);
t_dir							f_determine_direction(t_game *game,
									t_coords wall_hit);
t_int_xy						f_determine_adjacent_tile(t_game *game);
void							f_remove_obstacle(t_game *game);
t_int_xy						f_determine_tile_coords(t_game *game,
									t_coords grid_hit, t_dir dir);
void							f_add_obstacle_to_respawn_list(t_game *game,
									t_int_xy coords, char type);
